...
介绍一下 vue-router 在实战中的几个常见的功能。

首先是在**路由匹配的语法上**，vue-router 支持动态路由。例如我们有一个用户页面，这个页面使用的是 User 组件，但是每个用户的信息都不一样，需要给每一个用户配置单独的路由入口，这时就可以按下面代码中的样式来配置路由。
...
以上代码，冒号开头的 id 就是路由的动态部分，会同时匹配 /user/dasheng 和 /user/geektime，[这一部分详细内容可以参开官方文档的路由匹配语法部分](https://router.vuejs.org/zh/guide/essentials/route-matching-syntax.html#sensitive-%E4%B8%8E-strict-%E8%B7%AF%E7%94%B1%E9%85%8D%E7%BD%AE)

在开发中还会遇到类似这样的需求：对于一些页面来说，只有管理员才可以访问，普通用户访问时，会提示没有权限。这时就需要用到 vue-router 的**导航守卫功能**了，也就是子在访问路由页面前进行权限认证，这样可以做到对页面的控制，只允许某些用户可以访问。

在项目庞大之后，如果首屏加载文件太大，那么就可能会影响到性能。这个时候，我们可以使用 vue-router 的动态导入功能，**把不常用的路由组件单独打包，当访问到这个路由的时候再进行加载，这也是 vue 项目中常见的优化方式。**